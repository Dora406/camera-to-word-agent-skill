...
通过调用本地摄像头捕捉画面，并借助本地运行的 **Qwen3-VL:4B** 多模态视觉大模型(用户可根据电脑硬件情况自行更改模型，比如更改成qwen3-vl:30b等更大的模型，效果会更好)，该技能可以将眼前的任何纸质合同、发票、书籍、打印文稿等实体材料，在秒级时间内完成“高精度中文OCR识别 + 标题/正文结构化排版”，并直接在用户的电脑桌面上输出一份排版精美的 `.docx` (Word) 文档。
